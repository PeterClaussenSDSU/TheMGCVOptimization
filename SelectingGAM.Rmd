...
Compare the `mgcv` bases with similar basis from other libraries, starting with `splines`.
```{r}
library(splines)
...
According to the documentation, `splines2` provides for additional spline bases, and that the `bSpline` function extends the `bs` from `splines`. As we see below for ordinary cubic splines, there is no difference in the bases.
