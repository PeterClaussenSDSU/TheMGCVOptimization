...
```{r}
plot(x,X[,1],ylim=c(-.5,1),xlim=c(-10,150),type='l',col=1)
for(i in 2:dim(X)[2]) {
  lines(x,X[,i],col=i)
}
```
...
```{r}
gam14.smooth.bs$smooth[[1]]$knots
```
...
We can simplify this and compute an unpenalized fit by $\widehat{\beta} = \left(\mathbf{X}^t \mathbf{X}\right)^{-1} \mathbf{X}^t \mathbf{y}$.
...
lines(x,y14.hat)
```
Note that this fit is more 'wiggly' when compared to the penalized fits from `mgcv`.
...
gam18.smooth.bs$sp
```
We find very different $\lambda$ for these data, even though the data points nearly overlap.
...
S
image(S)
plot(S[,1],ylim=c(-.5,.5),type='l',col=1)
for(i in 2:dim(S)[2]) {
  lines(S[,i],col=i)
}
```

According to Google, "D matrix mgcv", a D or design matrix can be obtained by calling `model.matrix.gam`. But this isn't quite right, it looks like the `X` matrix from above.

```{r}
D <- model.matrix.gam(gam14.smooth.bs)
plot(x,D[,1],ylim=c(-.5,1),type='l',col=1)
for(i in 2:dim(D)[2]) {
  lines(x,D[,i],col=i)
}
t(D) %*% D
...
There is, however a `D` matrix is returned as part of the `smooth` list. When square, `D` produces a matrix of the correct dimensions.
...
```{r}
plot(D[,1],ylim=c(-.02,.01), type='l',col=1)
for(i in 2:dim(D)[2]) {
  lines(D[,i],col=i)
}
```

Compare this to the code from creating a `D` matrix from a penalized piecewise linear smoother, as shown in the function `prs.fit` on p 169. 

First, we need to know how many knots. This can be extracted from the `mgcv` object using `$knots`.

```{r}
gam14.smooth.bs$smooth[[1]]$knots
length(gam14.smooth.bs$smooth[[1]]$knots)
```

Now, the code from `prs.fit`:

```{r}
xj <- seq(min(x),max(x),length=8)
xj
D.169 <- diff(diag(length(xj)),differences=2)
plot(D.169[,1],ylim=c(-2,1), type='l',col=1)
for(i in 2:dim(D.169)[2]) {
  lines(D.169[,i],col=i)
}
```

This $\mathbf{D}$ looks reasonable, but looks very different than the `$D` member variable returned from `mgcv` as described above.
...
But if we substitute `D`, (using values of $\lambda$ from the `mgcv` solutions) then, for the 214m data set (the data for which `mgcv` finds a non-linear curve), we find:
...
Now, for a larger $\lambda$ as found for the 216m data set (but using the same $\mathbf{y}$ from 214m) :
...
Finally, the very large $\lambda$ found for the 218m data set:
```{r}
lambda = 885002639564
beta.hat <- solve(t(X) %*% X + lambda*S) %*% (t(X) %*% y14)
y14.hat <- X %*% beta.hat
...
The first plot looks reasonable, but the second two do not produce a linear trend as expected. Next step is to determine what I'm missing. Looking back, we see that the $\mathbf{S}$ matrix is very different from what we found in Python.

```{r}
plot(S[,1],ylim=c(-0.001,.001), type='l',col=1)
for(i in 2:dim(S)[2]) {
  lines(S[,i],col=i)
}
```

So, what does the $D$ look like?
```{r}
plot(D[,1],ylim=c(-.03,.03), type='l',col=1)
for(i in 2:dim(D)[2]) {
  lines(D[,i],col=i)
}
```
Note the magnitude of these columns; the numbers are very small compared with what we find in Python.
I don't think the `D` returned with an `mgcv` object is the appropriate `D` for the linear algebra we're attempting here.
...
It may be that the GCV algorithm is forcing $\lambda$ to be unreasonable. But does this affect the coefficients for the bases?
...
gam16.smooth.bs$weights
gam18.smooth.bs$weights
```
